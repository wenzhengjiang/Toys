...
conn_sendpkt (conn_t *c, const packet_t *pkt, size_t len)
{
    ...
    if (n < 0) {
        ...
        fprintf(stderr, "Didn't send the complete packet");
    }
    return n;
}
...
conn_poll (const struct config_common *cc)
{
    ...
    /*revents is true if the event occurred */
    ...
    for (;;) {
        ...
        if (cevents[0].revents) {
            ...
            if ((u = connect_to (1, &cc->server)) >= 0) {
                c = conn_alloc ();
                /* rfd and wfd connected to stream TCP connection */
                /* nfd connected to server connection */
                c->rfd = s;
                c->wfd = s;
                c->nfd = u;
                c->peer = cc->server;
